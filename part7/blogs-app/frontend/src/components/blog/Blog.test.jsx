...
describe("<Blog />", () => {
  ...
  test("renders title, author but does not render Url, likes", () => {
    ...
    expect(screen.queryByText("Click here to see full blog")).not.toBeInTheDocument();
  });
  ...
  test("renders url and number of likes when view button clicked", async () => {
    ...
    expect(screen.queryByText("Click here to see full blog")).toBeInTheDocument();
  });
  ...
  test("handle likes is called twice if like button is clicked twice", async () => {
    ...
    render(<Blog blog={testBlog} onUpdateBlog={handleLikes} onDeleteBlog={() => {}} />);
